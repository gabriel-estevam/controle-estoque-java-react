...
import { Button, LinearProgress, Pagination, Table, TableBody, TableCell, TableFooter, TableHead, TableRow, Box, Grid, Alert, AlertTitle, Collapse, IconButton, } from '@mui/material';
...
export const MovimentacaoEstoque = () => {
    ...
    const [dadoEstoque, setDadoEstoque] = useState();
    ...
    return (<LayoutBasePagina renderTabela titulo="Movimentação de Estoque" subTitulo="Materiais em Estoque" totalElements={(totalElements >= 0 && totalElements < 3 ? 32 : 62) || (totalElements > 5 ? 65 : 65)} barraFerramentas={<BarraFerramentas mostrarInputBusca mostrarBotaoNovo={false} textoDaBusca={busca} aoMudarTextoDeBusca={texto => setSearchParams({ busca: texto, pagina: '1', paginaAPI: '0' }, { replace: true })}/>}>
                ...
                <Table>
                    ...
                    <TableBody>
                        {rows.map(row => (<TableRow key={row.idEstoque}>
                                ...
                                <TableCell width={200}>
                                    <Button variant="contained" color="primary" disableElevation onClick={() => { handleOpen(); getEstoqueById(row.idEstoque); }}>
                                        Movimentar Item
                                    </Button>
                                </TableCell>
                            </TableRow>))}
            ...
            <ModalCadastro open={openModal} handleClose={handleClose} formSubmit={save} titulo="Movimentação de Estoque" tituloButtonAdd="Movimentar Estoque" heightDialog="300px">
                ...
                <Box margin={1} display="flex" flexDirection="column">
                    <Grid container direction="column" padding={2} spacing={2}>
                        <Grid container item direction="row" spacing={1}>
                            <Grid item md={3}>
                                <VTextField name="quantidade" label="Qtde.Atual" variant="outlined" type="number"/>
                            </Grid>
                        ...
                        <Grid container item direction="row" spacing={3}>
                            ...
                            <Grid item md={6}>
                                <VDateTimeField disabled name="data" label="Data de Movimentação" variant="outlined" type="text" sx={{
            "& input": { border: 'none',
                margin: 2,
                padding: 1,
                paddingY: 0,
