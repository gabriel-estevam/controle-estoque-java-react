...
import { Button, LinearProgress, Pagination, Table, TableBody, TableCell, TableFooter, TableHead, TableRow, Box, Grid, Alert, AlertTitle, Collapse, IconButton, TextField } from '@mui/material';
import TableContainer from '@mui/material/TableContainer';
import * as yup from 'yup';
import { VDateTimeField, VForm, VTextField, useVForm } from '../../forms';
...
import { EstoqueEntradaService } from '../../services/api/estoque/EstoqueEntrada';
const formValidationSchema = yup.object().shape({
    ...
    itensEstoque: yup.array(yup.object({
...
export const EstoqueEntrada = () => {
    ...
    return (<LayoutBasePagina renderTabela titulo="Administração de Materiais" subTitulo="Gerenciamento de Materiais" totalElements={(totalElements >= 0 && totalElements < 3 ? 32 : 62) || (totalElements > 5 ? 65 : 65)} barraFerramentas={<BarraFerramentas textoBotaoNovo="NOVO MATERIAL" mostrarInputBusca mostrarBotaoNovo textoDaBusca={busca} aoClicarEmNovo={handleOpen} aoMudarTextoDeBusca={texto => setSearchParams({ busca: texto, pagina: '1', paginaAPI: '0' }, { replace: true })}/>}>
            <TableContainer sx={{ height: "500px", margin: 1 }}>
                <Table>
                    ...
                    <TableBody>
                        {rows.map(row => (<TableRow key={row.idEstoque}>
                               <TableCell>{new Date(row.dataEntrada).toLocaleString().replace(/,/, '')}</TableCell>
                                ...
                                <TableCell width={200}>
                                    <Button variant="contained" color="error" disableElevation onClick={() => { handleDelete(row.idEstoque); }}>
                                        Deletar
                                    </Button>
                                </TableCell>
                            </TableRow>))}
            ...
            <ModalCadastro open={openModal} handleClose={handleClose} formSubmit={save} titulo="Adicionar novo Material ao Estoque" tituloButtonAdd="Adicionar Material" heightDialog="300px">
                ...
                <Box margin={1} display="flex" flexDirection="column">
                    <Grid container direction="column" padding={2} spacing={2}>
                        ...
                        <Grid item md={4}>
                            <VDateTimeField disabled name="dataEntrada" label="Data de Entrada" variant="outlined" type="text" sx={{
            "& input": { border: 'none',
                margin: 2,
                padding: 1,
                paddingY: 0,
                paddingRight: 0
                            ...
                            <Grid item md={3}>
                                <VTextField name="itensEstoque[0].quantidadeAtual" label="Qtde.Atual" variant="outlined" type="number"/>
                            </Grid>

                            <Grid item md={3}>
                                <VTextField name="itensEstoque[0].quantidadeMinima" label="Qtde.Minima" variant="outlined" type="number"/>
                            </Grid>

                            <Grid item md={3}>
                                <VTextField name="itensEstoque[0].quantidadeMaxima" label="Qtde.Maxima" variant="outlined" type="number"/>
                            </Grid>

                            <Grid item md={3}>
                                <VTextField name="itensEstoque[0].quantidadeIdeal" label="Qtde.Ideal" variant="outlined" type="number"/>
                            </Grid>
                        ...
                        <Grid container item direction="row" spacing={3}>
                            ...
                            <Grid item md={6}>
                                <TextField disabled value={usuarioToken.name} name="UsuarioFK" label="Usuario" variant="outlined" type="text" sx={{
            "& input": { border: 'none',
                margin: 2,
                padding: 1,
                paddingY: 0,
