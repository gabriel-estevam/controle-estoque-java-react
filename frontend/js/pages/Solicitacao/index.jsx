...
export const Solicitacao = () => {
    ...
    const [rowsItem, setRowsItens] = useState([]);
    ...
    const removeItem = (id) => {
        ...
        const filteredItem = newItens.filter((item) => item.produtoFK !== id ? item : null);
    ...
    return (<LayoutBasePagina renderTabela titulo="Requisição de Materiais" subTitulo="Materiais Requisitados" totalElements={(totalElements >= 0 && totalElements < 3 ? 32 : 62) || (totalElements > 5 ? 65 : 65)} barraFerramentas={<BarraFerramentas textoBotaoNovo="NOVA SOLICITAÇÃO" mostrarInputBusca mostrarBotaoNovo textoDaBusca={busca} aoClicarEmNovo={handleOpen} aoMudarTextoDeBusca={texto => setSearchParams({ busca: texto, pagina: '1', paginaAPI: '0' }, { replace: true })}/>}>
                        ...
                        <Grid container item direction="row" spacing={1}>
                            ...
                            <TableContainer>
                                <Table>
                                    ...
                                    <TableBody>
                                        {rowsItem.map(row => (<TableRow key={row.produtoFK}>
                                                <ItemSolicitacao item={row} buttonRm={<Button variant="contained" color="error" disableElevation onClick={() => removeItem(row.produtoFK)}>
                                                            Remover
                                                        </Button>}/>                                         
                                            </TableRow>))}
                                    </TableBody>
                                    {rowsItem.length === 0 && (<caption>Lista Vazia</caption>)}
                                </Table>
                            </TableContainer>
                        </Grid>
                    </Grid>
            ...
            <ModalSolicitacao open={openModalEdit} handleClose={handleCloseEdit} heightDialog="300px" solicitacao={solicitacao}/>
