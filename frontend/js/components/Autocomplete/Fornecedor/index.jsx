...
export const AutoCompleteFornecedor = ({ name, isExternalLoading = false, isMovimentoEstoque = false, isEdit }) => {
    ...
    return (<Autocomplete openText='Abrir' closeText='Fechar' noOptionsText='Sem opções' loadingText='Carregando...' disablePortal options={opcoes} loading={isLoading} disabled={isExternalLoading || isMovimentoEstoque} value={autoCompleteSelectedOption} onInputChange={(_, newValue) => setBusca(newValue)} onChange={(_, newValue) => { setSelectedId(newValue?.idFornecedor); clearError(); }} popupIcon={(isExternalLoading || isLoading) ? <CircularProgress size={28}/> : undefined} renderInput={(params) => (<TextField {...params} label="Fornecedor" error={!!error} helperText={error} sx={{
                "& input": {
                    border: 'none',
                    margin: 0,
                    padding: 1,
